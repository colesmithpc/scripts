...
A cross-platform Python utility that flushes the DNS cache on **Windows**, **Linux**, and **macOS**.  
Designed for troubleshooting, networking diagnostics, privacy, and system reset workflows.

This script automatically detects your operating system and uses the correct method to clear DNS caches, including various service managers on Linux.
...
- ✅ Detects and uses correct DNS flushing commands per OS  
- ✅ Handles multiple Linux DNS services (`systemd-resolved`, `dnsmasq`, `nscd`, etc.)  
- ✅ Warns if not running with admin/root privileges  
...
### **Linux or macOS (recommended with sudo)**
```bash
sudo python dns_flush.py
```
...
| **Linux** | ✔ | Tries multiple DNS service managers |
...
Below is the full script included in this repository:
...
✅ add logging  
or  
✅ package it as a CLI tool (`pip install dnsflusher`)  
